...
class FactualStatement(BaseModel):
    ...
    statement: str = Field(..., description="Declaração factual extraída do documento")
    context: str = Field(..., description="Contexto ou seção do documento de onde a declaração foi extraída")

class ExtractedFacts(BaseModel):
    """Modelo para representar a lista de declarações factuais extraídas de um documento."""
    facts: List[FactualStatement] = Field(..., description="Lista de declarações factuais extraídas")
...
def main():
    ...
    log_file_path = os.path.join(FINE_TUNING_DIR, "llm_interactions.log")
